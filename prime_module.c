...
void mark_primes(int *counter){
	...
	while(1){
		...
		*/
		...
		while(primes[position] == 0 && position < upper_bound-1){
			position++;
		}
		spin_unlock(&lock1);
...
static int prime_init(void){
	...
	primes = kmalloc((upper_bound-1)*sizeof(int), GFP_KERNEL); //FIXED
	if(primes == NULL){
		/* error */
...
MODULE_DESCRIPTION ("CSE422:Lab2 Concurrent Memory Use");
